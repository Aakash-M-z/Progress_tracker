    ...
    return (
        <div className="min-h-screen bg-[#07070a] text-slate-200 flex flex-col items-center justify-center p-4 sm:p-8 relative overflow-hidden">
            ...
            <motion.div
                ...
                className="w-full max-w-xl bg-[#0c0d16]/95 border border-indigo-500/20 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-2xl space-y-6 relative z-10 text-center"
            >
                {/* Status Hero Icon */}
                <div className="w-16 h-16 rounded-3xl bg-gradient-to-br from-[#D4AF37] to-[#AA8A2A] text-black flex items-center justify-center mx-auto text-2xl shadow-xl shadow-[#D4AF37]/30">
                ...
                <div>
                    ...
                    <h1 className="text-2xl sm:text-3xl font-black text-white">
                        {report?.passed ? 'Congratulations!' : 'Assessment Submitted!'}
                    </h1>
                ...
                {!showResults ? (
                    ...
                    <>
                        {/* Score Hero Banner */}
                        <div className="p-5 rounded-2xl bg-gradient-to-br from-white/[0.04] to-black border border-white/10 space-y-3">
                            ...
                            <div className="text-4xl font-black text-white">
                                {report?.score} <span className="text-lg text-slate-500 font-normal">/ {report?.maxScore}</span>
                            </div>
                        ...
                        <div className="grid grid-cols-3 gap-2.5 text-center">
                            <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5">
                                <span className="text-[0.65rem] uppercase font-bold text-slate-500 block">Accuracy</span>
                                <strong className="text-sm text-indigo-400">{report?.accuracy}%</strong>
                            </div>
                            <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5">
                                <span className="text-[0.65rem] uppercase font-bold text-slate-500 block">Questions Solved</span>
                                <strong className="text-sm text-emerald-400">{report?.correctCount} / {report?.totalQuestions}</strong>
                            </div>
                            <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5">
                                <span className="text-[0.65rem] uppercase font-bold text-slate-500 block">Time Taken</span>
                        ...
                        {report?.categoryScores && (
                            <div className="text-left space-y-2 pt-2">
                                <h5 className="text-[0.7rem] uppercase font-bold text-slate-400 tracking-wider">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {Object.entries(report.categoryScores).map(([cat, data]: [string, any]) => (
                                        <div key={cat} className="p-2.5 rounded-xl bg-white/[0.02] border border-white/5 flex items-center justify-between text-xs">
                                            ...
                                            <strong className="text-indigo-400 font-bold ml-2">{data.percentage}%</strong>
                                        </div>
                                    ))}
                                </div>
                            </div>
                ...
                <div className="pt-3 flex gap-3">
                    ...
                    <button
                        ...
                        className="flex-1 py-3 rounded-xl bg-gradient-to-r from-[#D4AF37] to-[#AA8A2A] text-black font-black text-xs hover:brightness-110 shadow-lg shadow-[#D4AF37]/20 flex items-center justify-center gap-1.5 transition-all"
                    >
                        My Assessments <ArrowRight className="w-4 h-4" />
                    </button>
                </div>
            </motion.div>
        </div>
